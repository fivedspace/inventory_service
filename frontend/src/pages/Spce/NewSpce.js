...
export default function MerchantProfile(props) {
    ...
    function subUpdate(res_data){
        if((spec_nameItem.error && spec_remarkItem.error) || (!spec_nameItem.value && !spec_remarkItem.value) ){
            ...
            axios.patch(config.spec1,{"spec_id":2,"spec_name": spec_nameItem.value,"data_type_id":4, "spec_remark": spec_remarkItem.value},{headers:{}})
                .then((res)=>{
                    if(res){
                        console.log("修改规格信息成功！");
                        flag = !flag
                        setPageType("修改系统信息")
                        clearVariable();
                    }
