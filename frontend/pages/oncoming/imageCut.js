...
export default function imageCut(props) {
  ...
  return (
    <form className={classes.root} noValidate autoComplete="off">
      ...
      <TextField id="filled-basic" label="属性类型" variant="filled" disabled value={item.type} />
      <>
        <input
          accept="image/*"
        ...
        <label htmlFor={ item.id ? item.id : item.uuid }>
          <Button variant="contained" color="primary" component="span" style={{ display: amend ? 'none' : 'inline-block' }}>选择文件</Button>
        </label>
      </>
